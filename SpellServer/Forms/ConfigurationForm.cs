...
namespace SpellServer.Forms
{
	public partial class ConfigurationForm : Form
	{
        ...
        {
            if (UDPPortTextBox.Text.IsNumber(false))
            {
                Int32 port;
                Int32.TryParse(UDPPortTextBox.Text, out port);

                if (port > 65535)
                {
                    ConfigToolTip.SetToolTip(UDPPortTextBox, "You must set a valid range from 1 to 65535.");
                    ConfigToolTip.ToolTipIcon = ToolTipIcon.Error;
                    ConfigToolTip.ToolTipTitle = "Invalid Port Range";
                    UDPPortTextBox.Text = @"10601";
        ...
        private void UDPPortTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!e.KeyChar.ToString(CultureInfo.CurrentCulture).IsNumber(false) && e.KeyChar != (Char)Keys.Back)
            {
                ConfigToolTip.SetToolTip(ListenPortTextBox, "This box may only contain numbers.");
		...
		private void SaveConfigurationButton_Click(object sender, EventArgs e)
		{
            ...
            Settings.Default.UDPPort = Int32.TryParse(UDPPortTextBox.Text, out port) ? port : 10601;
